...
function jQmain() {
	 ...
	 * @license		MIT License
	 */
	(function($){$.support.fullscreen=supportFullScreen();$.fn.fullScreen=function(props){if(!$.support.fullscreen||this.length!=1){return this}if(fullScreenStatus()){cancelFullScreen();return this}var options=$.extend({'background':'#111','callback':function(){}},props);var fs=$('<div>',{'css':{'overflow-y':'auto','background':options.background,'width':'100%','height':'100%'}});var elem=this;elem.addClass('fullScreen');fs.insertBefore(elem);fs.append(elem);requestFullScreen(fs.get(0));fs.click(function(e){if(e.target==this){cancelFullScreen()}});elem.cancel=function(){cancelFullScreen();return elem};onFullScreenEvent(function(fullScreen){if(!fullScreen){elem.removeClass('fullScreen').insertBefore(fs);fs.remove()}options.callback(fullScreen)});return elem};function supportFullScreen(){var doc=document.documentElement;return('requestFullscreen'in doc)||('mozRequestFullScreen'in doc&&document.mozFullScreenEnabled)||('webkitRequestFullScreen'in doc)}function requestFullScreen(elem){if(elem.requestFullscreen){elem.requestFullscreen()}else if(elem.mozRequestFullScreen){elem.mozRequestFullScreen()}else if(elem.webkitRequestFullScreen){elem.webkitRequestFullScreen()}}function fullScreenStatus(){return document.fullscreen||document.mozFullScreen||document.webkitIsFullScreen}function cancelFullScreen(){if(document.exitFullscreen){document.exitFullscreen()}else if(document.mozCancelFullScreen){document.mozCancelFullScreen()}else if(document.webkitCancelFullScreen){document.webkitCancelFullScreen()}}function onFullScreenEvent(callback){$(document).on("fullscreenchange mozfullscreenchange webkitfullscreenchange",function(){callback(fullScreenStatus())})}})(jQuery);
	...
	// Create slideshow HTML and insert
	var slideshow_html = $('			\
		<!--Arrow Navigation-->\
	...
	// Set up Supersized and default Shutter theme once relevant HTML inserted
	/*
	...
	*/
	...
	// Forch theme: MODIFIED Shutter theme to remove thumb and navigation stuff, and use playpng and pausepng vars
	...
	var show_slideshow_html = function(speed){
		...
		$("#supersized").show(speed);
		//$("#supersized").animate({"display":"block-inline", speed);
		$("#controls-wrapper").show(speed);
		//$("#thumb-tray").show(speed);
		//$("#nextthumb, #prevthumb").show(speed);
		$("#prevslide, #nextslide").show(speed);	
	};

	var hide_slideshow_html = function(){
		$("#supersized").hide("fast");
		$("#controls-wrapper").hide("fast");
		$("#thumb-tray").hide("fast");
		$("#nextthumb, #prevthumb").hide("fast");
		$("#prevslide, #nextslide").hide("fast");	
		// $("#supersized").remove();
		// $("#controls-wrapper").remove();
		// $("#thumb-tray").remove();
		// $("#nextthumb, #prevthumb").remove();
	...
	// Attach callback
	$("#chan_start_slideshow").click(start_slideshow);
}
